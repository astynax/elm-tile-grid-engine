...
Elm.TileGrid.make = function (_elm) {
   "use strict";
   _elm.TileGrid = _elm.TileGrid || {};
   if (_elm.TileGrid.values) return _elm.TileGrid.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   var _op = {};
   var combine = F2(function (u1,u2) {
      var $try = F2(function (f,x) {    return A2($Maybe.withDefault,x,f(x));});
      return function (_p0) {
         return $Maybe.Just(A2($try,u2,A2($try,u1,_p0)));
      };
   });
...
Elm.State.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var isPosInBounds = function (s) {
      var row = $Matrix.row(s.pos);
      var col = $Matrix.col(s.pos);
      return _U.cmp(col,0) > -1 && (_U.cmp(row,0) > -1 && (_U.cmp(col,$Matrix.colCount(s.map)) < 0 && _U.cmp(row,$Matrix.rowCount(s.map)) < 0));
   };
   var isPosValid = F2(function (p,s) {    return A2($Maybe.withDefault,false,A2($Maybe.map,p,A2($Matrix.get,s.pos,s.map)));});
   var move = F2(function (d,s) {    return _U.update(s,{pos: A2($Matrix.loc,$Matrix.row(s.pos) - d.y,$Matrix.col(s.pos) + d.x)});});
   var moveTo = F3(function (p,d,s) {    var s$ = A2(move,d,s);return isPosInBounds(s$) && A2(isPosValid,p,s$) ? $Maybe.Just(s$) : $Maybe.Nothing;});
   var toMap = function (s) {    return $Matrix.toList(A3($Matrix.set,s.pos,s.player,s.map));};
   var State = F3(function (a,b,c) {    return {map: a,pos: b,player: c};});
   var Direction = F2(function (a,b) {    return {x: a,y: b};});
   return _elm.State.values = {_op: _op,toMap: toMap,moveTo: moveTo,State: State,Direction: Direction};
};
Elm.Sparkle = Elm.Sparkle || {};
Elm.Sparkle.make = function (_elm) {
   "use strict";
   _elm.Sparkle = _elm.Sparkle || {};
   if (_elm.Sparkle.values) return _elm.Sparkle.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var _op = {};
   var andAfter = F2(function (evo,update) {    return A2($Signal.map,F2(function (u,state) {    return A2($Maybe.map,update,u(state));}),evo);});
   var timesPerSec = F2(function (fps,update) {    return A2($Signal.map,F2(function (_p0,state) {    return $Maybe.Just(update(state));}),$Time.fps(fps));});
   var movement = function () {
      var autorepeat = $Signal.sampleOn($Time.fps(5));
   ...
   var playerBlinking = function (state) {
      ...
      {player: function () {
         var _p3 = state.player;
         if (_p3.ctor === "Player") {
               return Player($Basics.not(_p3._0));
            } else {
               return state.player;
            }
      }()});
   ...
   var initialState = {map: A3($Matrix.matrix,30,40,function (_p4) {    return Floor;}),pos: A2($Matrix.loc,5,5),player: Player(true)};
   var Fire = function (a) {    return {ctor: "Fire",_0: a};};
   var flameDying = function (state) {
      return _U.update(state,
      {map: A2($Matrix.map,
      function (cell) {
         var _p5 = cell;
   ...
   return _elm.Sparkle.values = {_op: _op
                                ...
                                ,andAfter: andAfter
                                ,putFlameAtPlayerPos: putFlameAtPlayerPos};
};
